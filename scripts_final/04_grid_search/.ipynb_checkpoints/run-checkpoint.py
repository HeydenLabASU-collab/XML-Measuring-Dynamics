...
def generate_grid_search_plot(clf, max_depth, n_estimatpors, min_samples_leaf = 0.01):
    df = pd.concat([pd.DataFrame(clf.cv_results_["params"]), pd.DataFrame(clf.cv_results_["mean_train_score"], columns=["Train Accuracy"]), pd.DataFrame(clf.cv_results_["std_train_score"], columns=["Train STD"]), pd.DataFrame(clf.cv_results_["mean_test_score"], columns=["Test Accuracy"]), pd.DataFrame(clf.cv_results_["std_test_score"], columns=["Test STD"])], axis=1)
    df_query = df[(df["min_samples_leaf"] == 0.01)]
    ...
    im = ax[0].imshow(train_accuracy_arr, cmap = colorbar)
    im = ax[1].imshow(test_accuracy_arr, cmap = colorbar)
    ...
    for p in range(2):
        ...
        for i in range(len(max_depth)):
            for j in range(len(n_estimators)):
                if p == 0:
                    text = ax[p].text(j, i, f"{np.round(train_accuracy_arr[i, j],3)}", ha="center", va="center", color="b")
                    ax[p].set_title("Training Data Grid Search")
                if p == 1:
                    text = ax[p].text(j, i, f"{np.round(test_accuracy_arr[i, j],3)}", ha="center", va="center", color="b")
                    ax[p].set_title("Test Data Grid Search")
        ax[p].set_ylabel("Max Depth")
        ax[p].set_xlabel("Number of Estimators")
    fig.tight_layout()
    
    df.to_csv('gridsearch_allparams.csv', index=False)
    np.savetxt(f"training_accuracy.csv", train_accuracy_arr)
    np.savetxt(f"test_accuracy.csv", test_accuracy_arr)
    plt.savefig(f"GridSearchCV.jpg", dpi=400)

clf = run_grid_search(ncv, n_estimators, max_depth, X, y, w, min_samples_leaf = 0.01, n_jobs=1)
generate_grid_search_plot(clf, max_depth, n_estimators, min_samples_leaf = 0.01)